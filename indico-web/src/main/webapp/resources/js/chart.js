(function(L) {
            ...
            if (ap.show || this.scaleToHiddenSeries) {
                ...
                for (var ak = 0, ai = am.length; ak < ai; ak++) {
                    if (this.name == "xaxis" || this.name == "x2axis") {
                        ...
                        if (am[ak][ah] != "-" && ((am[ak][ah] != null && am[ak][ah] < ao.min) || ao.min == null)) {
                            ao.min = am[ak][ah]
                        }
                        if (am[ak][an] != "-" && ((am[ak][an] != null && am[ak][an] > ao.max) || ao.max == null)) {
                            ao.max = am[ak][an]
                        }
                    }
                }
    ...
    L.jqplot.extend = function() {
        ...
        // changed strictly comarisson because of sonnar report
    ...
    L.fn.jqplotToImageCanvas = function(options) {
    ...
    ag.prototype.diff = function(ai, al, ah) {
        ai = new ag(ai);
        // commented by DD due to sonar report -- code never reached since use of "new" object
        //if (ai == null) {
        //    return null
        //}
        var aj = E[al] || E.day;
    ...
    ag.prototype.getUnix = function() {
        return Math.round(this.proxy.getTime() / 1000)
    };
        ...
        if (this.show) {
            ...
            if (C.show) {
                ...
                for (H = 0; H < o; H++) {
                    ...
                    // a is always false in 9312, so this if has no sense / modified by DD due a sonar report
                    //if (A) {
                                ...
                                if (this.waterfall && I > 0 && I < this.gridData.length - 1) {
                                    ...
                                    if (this.waterfall && I == 0 && I < this.gridData.length - 1) {
                                        if (this._xaxis.min <= 0 && this._xaxis.max >= 0) {
                                        	...
                                        	// commented by DD, because of sonar report - same condition
...
(function(a) {
    ...
    a.jqplot.CanvasFontRenderer.prototype.measure = function(c, e) {
        var d = this.fontWeight + " " + this.fontSize + " " + this.fontFamily;
        c.save();
        c.font = d;
    ...
    a.jqplot.CanvasFontRenderer.prototype.draw = function(e, g) {
        ...
        var f = this.fontWeight + " " + this.fontSize + " " + this.fontFamily;
        e.font = f;
        e.translate(d, b);
        e.rotate(this.angle);
        e.fillText(g, c, h);
...
(function(j) {
    ...
    j.jqplot.Cursor.prototype.doZoom = function(G, t, C, u) {
    	var FM = C.axes.xaxis.tickInterval;
        var FT = C.axes.xaxis.numberTicks;
        var I = u;
        var F = C.axes;
        var r = I._zoom.axes;
        var w = r.start;
        ...
        if ((I.constrainZoomTo == "none" && Math.abs(G.x - I._zoom.start[0]) > 6 && Math.abs(G.y - I._zoom.start[1]) > 6) || (I.constrainZoomTo == "x" && Math.abs(G.x - I._zoom.start[0]) > 6) || (I.constrainZoomTo == "y" && Math.abs(G.y - I._zoom.start[1]) > 6)) {
            if (!C.plugins.cursor.zoomProxy) {
                ...
                if (FM != null && C.axes.xaxis.tickInterval==null && C.axes.xaxis.numberTicks==null) {
                	C.axes.xaxis.tickInterval=FM;
                	C.axes.xaxis.numberTicks=FT;
                } else if (C.axes.xaxis.tickInterval<1) {
                	C.axes.xaxis.tickInterval=FM;
                	C.axes.xaxis.numberTicks=FT;
                } 
                C.redraw();
                ...
                if (this.min == null && this.max == null) {
                    ...
                    if (!this.tickInterval && !this.numberTicks) {
                        ...
                        if (this.tickRenderer === h.jqplot.CanvasAxisTickRenderer && this.tickOptions.angle) {
                            Y = 115 - 40 * Math.abs(Math.sin(this.tickOptions.angle / 180 * Math.PI))
                        }
                    ...
                    if (I <= 19 * l) {
                        ...
                        if (I <= 9 * j) {
                            ...
                            var U = new h.jsDate(ad).setDate(1).setHours(0, 0, 0, 0);
                            ...
                            ab = Math.ceil(T / D) + 1;
                            this.min = U.getTime();
                            this.max = U.clone().add((ab - 1) * D, "month").getTime();
                            this.numberTicks = ab;
                            for (var Z = 0; Z < ab; Z++) {
                                if (Z === 0) {
                                    N.value = U.getTime()
                            ...
                            var U = new h.jsDate(ad).setMonth(0, 1).setHours(0, 0, 0, 0);
                            var z = new h.jsDate(J).add(1, "year").setMonth(0, 1).setHours(0, 0, 0, 0);
                            var K = z.diff(U, "year");
                            ab = Math.ceil(K / D) + 1;
                            this.min = U.getTime();
                            this.max = U.clone().add((ab - 1) * D, "year").getTime();
                            this.numberTicks = ab;
                            for (var Z = 0; Z < ab; Z++) {
                                if (Z === 0) {
                                    N.value = U.getTime()
                    ...
                    var V, C;
                    V = (this.min != null) ? new h.jsDate(this.min).getTime() : ad - B / 2 * (this.padMin - 1);
                    ...
                    for (var Z = 0; Z < this.numberTicks; Z++) {
                        ...
                        ac = ad.add(Z * this.daTickInterval[0], this.daTickInterval[1]).getTime();
                        var X = new this.tickRenderer(this.tickOptions);
                        if (!this.showTicks) {
                            X.showLabel = false;
                            X.showMark = false
    ...
    CONFIGURATORS: {
        ...
        line: {
            configure: function(a) {
                ...
                if (a.cfg.stackSeries && a.cfg.axes.xaxis.renderer !== $.jqplot.DateAxisRenderer) {
        ...
        bar: {
            configure: function(a) {
                ...
                if (a.cfg.orientation === "horizontal") {
                    ...
                    if (a.cfg.dataRenderMode === "key") {
                        a.cfg.data = [a.cfg.data];
                        a.cfg.axes = {
                            xaxis: {
...
PrimeFaces.widget.Chart = PrimeFaces.widget.DeferredWidget.extend({
    ...
    adjustLegendTable: function() {
        var f = this.jq.find("table.jqplot-table-legend"),
            e = f.find("tr.jqplot-table-legend");
        if (e.length > 1) {
            var a = f.find("tr.jqplot-table-legend:first"),
                b = f.find("tr.jqplot-table-legend:last"),
                d = a.children("td").length - b.children("td").length;
            for (var c = 0; c < d; c++) {
                b.append("<td></td>")
            }
        }

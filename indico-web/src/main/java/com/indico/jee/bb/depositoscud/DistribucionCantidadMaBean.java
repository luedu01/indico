...
@ManagedBean(name="distribucionCantidadMaBean")
@ViewScoped
public class DistribucionCantidadMaBean extends GraficaGeneralBean {

	private static final long serialVersionUID = 1L;

	private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(DistribucionCantidadMaBean.class);
	...
	public void exportAction(ActionEvent evt) {
		...
		try {
			if (getTipoaexportar()!=null) {
		    	...
		    	List<AnalisisPagosSaldosXHora>  resultado =  ServiceFacades.getInstance().getAnalisisPagosSaldosXHoraService().getDetalleExportarPeriodo(fechaInicial, fechaFinal,Integer.valueOf(getPeriodicidad()));		       
    ...
    private void createXMLFile (List<AnalisisPagosSaldosXHora> valores) {
    	...
    	try {
    		...
    		for (AnalisisPagosSaldosXHora ana : valores) {
    			...
    			totalValor    = totalValor + IndicoUtils.getInstance().assertDoubleMM(ana.getNumTotalMovimientos());
    			cell.setCellValue(IndicoUtils.getInstance().assertDoubleMM(ana.getNumTotalMovimientos()));
    		}
			Row row = sheet.createRow(rownum++);
            ...
            response.setHeader(CONTENT_DISPOSITION_LITERAL, "attachment; filename=indico_dist_cant.xls");
    ...
    private void createCSVFile(List<AnalisisPagosSaldosXHora> valores) {
        ...
        try {
            ...
            response.setHeader(CONTENT_DISPOSITION_LITERAL, "attachment; filename=indico_dist_cant.csv");
			output = response.getOutputStream();
        	String line = MessageFormat.format(FORMAT_LITERAL_2,"Fecha","Hora","Cantidad");
        	line = line+"\r\n";
			...
			if(valores!=null && !valores.isEmpty()) {
	        	for (AnalisisPagosSaldosXHora ana : valores) {
		        	line = MessageFormat.format(FORMAT_LITERAL_2,	        	
		    			...
		    			//String.valueOf(IndicoUtils.getInstance().assertDouble(ana.getNumTotalMovimientos()))
		        		String.format("%.0f", ana.getNumTotalMovimientos())
		    		);
		        	if (output!=null && line !=null) output.write((line + "\r\n").getBytes());
	    			totalValor    = totalValor + IndicoUtils.getInstance().assertDoubleMM(ana.getNumTotalMovimientos());
		        }//for
			}//if
        	line = MessageFormat.format(FORMAT_LITERAL_2,"Total","",String.format("%.0f", totalValor));
        	if (output!=null && line!=null) output.write((line + "\r\n").getBytes());
	        fc.responseComplete();
        } catch (IOException e) {
    		FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, ERROR_LITERAL, null));
    ...
    private void createSDMXFile(List<AnalisisPagosSaldosXHora> valores) {
        ...
        try {
            ...
            response.setCharacterEncoding(UTF8_LITERAL);
            response.setHeader(CONTENT_DISPOSITION_LITERAL, "attachment; filename=indico_dist_cant_sdmx.xml");
			output = response.getOutputStream();
			String resultado = HEAD + HEADER;
			for (AnalisisPagosSaldosXHora valor : valores ) {
				//System.out.println("Hora Liq->"+valor.getPk().getHoraLiquidacion());
				resultado = resultado .concat( MessageFormat.format(
					...
					String.format("%.2f", valor.getNumTotalMovimientos())
					/*, 
					String.format("%.2f", valor.getValorSaldo()), 

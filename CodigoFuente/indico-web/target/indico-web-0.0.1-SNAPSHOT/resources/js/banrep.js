...
function createplotmaxdistribucionvalor(name,data,tipo,periodo1,periodo2) {
	...
	for (var i = 0; i < data["SerieValores"].length; i++) {
		 var fec001 = data["SerieValores"][i][0];
		 var hora 	= parseInt(data["SerieValores"][i][1]);
		 var valor 	= data["SerieValores"][i][2];
		 if (fec001==fec_p1) {
			 for(var h=0; h<horas.length; h++){
				 if(hora==horas[h]){
					 serieP1[h]=[ hora , valor ];
...
function createplotmaxdistribucioncantidad(name,data,tipo,periodo1,periodo2) {
	...
	for (var i = 0; i < data["SerieCantidad"].length; i++) {
		 var fec001 = data["SerieCantidad"][i][0];
		 var hora 	= parseInt(data["SerieCantidad"][i][1]);
		 var valor 	= data["SerieCantidad"][i][2];
		 if (fec001==fec_p1) {
			 for(var h=0; h<horas.length; h++){
				 if(hora==horas[h]){
					 serieP1[h]=[ hora , valor ];

...
public class CompAnioSemestre implements Serializable {
    ...
    private List<CampoSelect> semestres;
	...
	public void init(){
			...
			semestres = CargaFiltros.getInstance().obtenerSemestre(anioSelected);
	}
	...
	public Date getFechaInicialSeleccionada(){
    	if ( IndicoUtils.getInstance().notNull(getAnioSelected())  &&
       		 IndicoUtils.getInstance().notNull(semestreSelected)   
       		) {
	           	Calendar cal = Calendar.getInstance();
	           	cal.set(Calendar.YEAR			, new Integer(getAnioSelected())  );
	           	if (semestreSelected.equals("01")) 			cal.set(Calendar.MONTH	 , 0 ); 
	           	else if (semestreSelected.equals("02")) 	cal.set(Calendar.MONTH	 , 6 );
	...
	public Date getFechaFinalSeleccionada(){
    	if ( IndicoUtils.getInstance().notNull(getAnioSelected())  		&&
	           	...
	           	//Inicia las asignaciones
   	           	cal.set(Calendar.YEAR			, new Integer(getAnioSelected())  );
   	           	if (semestreSelected.equals("01")) 			cal.set(Calendar.MONTH	 , 5 );
   	           	else if (semestreSelected.equals("02")) 	cal.set(Calendar.MONTH	 , 11 );
   	           	else cal.set(Calendar.MONTH	 	, 5 );
	...
	public void changeAnioAction(ValueChangeEvent evt) {
		...
		if (anio==null) return;
		semestres = CargaFiltros.getInstance().obtenerSemestre(anio);
	}

	public void changeSemestreAction(ValueChangeEvent evt) {
		/*
		 * for p:ajax listener
		 */
	}
	...
	public List<CampoSelect> getSemestres() {
		return semestres;
	}

	public void setSemestres(List<CampoSelect> semestres) {

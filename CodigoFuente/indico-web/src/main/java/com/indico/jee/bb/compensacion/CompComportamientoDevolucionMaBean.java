...
public class CompComportamientoDevolucionMaBean extends GraficaGeneralBean {
	...
	private final  String preparedDate = formatter1.format(date);
	
	private  final String HEADER = 	"	<Header>	\r\n" +
											" <ID>Indico - Comportamiento Histórico Del Canje Cheques Devoluciones</ID>	\r\n" +
											" <Test>false</Test>													\r\n" +
											" <Truncated>false</Truncated>										\r\n" +
											" <Name xml:lang=\"es\">Indico - Comportamiento Histórico Del Canje Cheques Devoluciones</Name>	\r\n" +
											...
											" </Header>	\r\n" +
											" <DataSet>	\r\n" +
											" <generic:KeyFamilyRef>indico_Comportamiento_Histórico_Del_Canje_Cheques_Devoluciones</generic:KeyFamilyRef>	\r\n" +
    ...
    public void changeTipoPlazaAction(ValueChangeEvent evt) {
    	...
    	if (TODAS_LITERAL.equals(tipodeplaza)) {
    		...
    		//cargar ciudades
    		Map<String,String> valores = ServiceFacades.getInstance().getAnalisisChequesCompensaService().getCiudadesDevol(tipodeplaza);
		...
		try {
			if (getTipoaexportar()!=null) {
		        ...
		        if (tipodeplaza.equals(TODAS_LITERAL)) {
		        	resultado =  ServiceFacades.getInstance().getAnalisisChequesCompensaService().getDataDiarioDevolTodas(fechaInicial, fechaFinal, Integer.valueOf(getPeriodicidad()) );
		        } else {
		        	resultado =  ServiceFacades.getInstance().getAnalisisChequesCompensaService().getDataDiarioDevol(fechaInicial, fechaFinal, getTipodeplaza(), Integer.valueOf(getPeriodicidad())  );
		        }
    ...
    private void createXMLFile (List<AnalisisChequesCompensa> valores) {
    	...
    	try {
    		wb = new HSSFWorkbook();
    		Sheet sheet = wb.createSheet("indico_comportamiento_devolucion");
            ...
            response.setHeader(CONTENT_DISPOSITION_LITERAL, "attachment; filename=indico_comportamiento_devolucion.xls");
    ...
    private void createCSVFile(List<AnalisisChequesCompensa> valores) {
        ...
        try {
            ...
            response.setHeader(CONTENT_DISPOSITION_LITERAL, "attachment; filename=indico_comportamiento_devolucion.csv");
			output = response.getOutputStream();
    ...
    private void createSDMXFile(List<AnalisisChequesCompensa> valores) {
        ...
        try {
                ...
                response.setHeader(CONTENT_DISPOSITION_LITERAL, "attachment; filename=Comportamiento_historico_del_canje_cheques_devoluciones_dmx.xml");
    			output = response.getOutputStream();
